...
func (n *Namespace) RestoreInterfaces(ctx context.Context, interfaces map[Iface][]IfaceOption) error {
	...
	for iface, opts := range interfaces {
		...
		if i.dstName != "" {
			...
			if err != nil {
				log.G(ctx).WithError(err).WithField("interface", i.dstName).Warn("Failed to get link for neighbor advertisement during restore")
				continue
			}
			ifIndex := link.Attrs().Index
			// Update the MAC address from the actual interface in case it differs
			// from what was stored.
			if mac := link.Attrs().HardwareAddr; len(mac) > 0 {
				i.mac = mac
			}
